...
from models_cd import *
from models_cd.swin_transformer_v2 import tiny_swin_v2_cd
...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if args.net_G == 'mixmim_tiny_swin_v2':
        net = BASE_Transformer(args, input_nc=3, output_nc=2, token_len=4, resnet_stages_num=4,
                             with_pos='learned')
    else:
...
class Backbone(torch.nn.Module):
    def __init__(self, args, input_nc, output_nc,
        ...
        if args.net_G == 'mixmim_tiny_swin_v2':
            ...
            self.backbone = tiny_swin_v2_cd(
                img_size=args.img_size
            )

            if args.backbone_ckpt is not None:
            
                from misc.swin_utils import load_pretrained
                
                load_pretrained(args.backbone_ckpt, self.backbone)
            
            filters0 = [32, 64, 128, 256]
