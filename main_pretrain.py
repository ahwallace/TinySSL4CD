...
from utils_pretrain.pos_embed import interpolate_pos_embed
from utils_pretrain.misc import NativeScalerWithGradNormCount as NativeScaler
...
def get_args_parser():
    ...
    parser.add_argument('--low_freq_target', action='store_true',
                        help='Use low frequency reconstruction target as targets for computing loss')
    parser.set_defaults(norm_pix_loss=False)

    # Optimizer parameters
    parser.add_argument('--opt', type=str, default='AdamW', metavar='OPTIMIZER',
                        help='optimizer (opt)')
    parser.add_argument('--momentum', type=tuple, default=(0.9, 0.95), metavar='MOMENTUM PARAMS',
...
def main(args):
    ...
    dataset_train = LevirDataset(args.data_path, 
        split='train', 
        pretrain=True
    )
    print(dataset_train)
    ...
    model.to(device)
    # print(f'encoder cuda device? {next(model.encoder.parameters()).is_cuda}')
    # print(f'mixmim cuda device? {next(model.parameters()).is_cuda}')
    
    model.eval()
    flops = FlopCountAnalysis(model, torch.rand(10, 3, args.input_size, args.input_size).to(device)) 
    ...
    optimizer = optim_factory.create_optimizer_v2(model_without_ddp,
        opt=args.opt,
        lr=args.lr,
